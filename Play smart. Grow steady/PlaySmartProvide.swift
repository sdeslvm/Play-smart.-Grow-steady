...
protocol PlaySmartGradientProviding {
...
final class PlaySmartGradientContainerView: UIView, PlaySmartGradientProviding {
...
struct PlaySmartWebViewBox: UIViewRepresentable {
    // MARK: - Свойства
    
    @ObservedObject var loader: PlaySmartWebLoader
    
    // MARK: - Координатор
    
    func makeCoordinator() -> PlaySmartWebCoordinator {
        PlaySmartWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = PlaySmartGradientContainerView()
