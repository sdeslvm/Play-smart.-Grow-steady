...
struct PlaySmartGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://plasmigostead.top/play")! }
    
    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            PlaySmartEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}
